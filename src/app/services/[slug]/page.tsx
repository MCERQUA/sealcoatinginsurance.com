...
import {
  ...
  Umbrella, Droplets, Package, Paintbrush,
          ...
          <div className="container-wide">
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <FadeIn className="lg:col-span-7">
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-clay-gradient text-espresso mb-5 shadow-warm"><Icon className="h-7 w-7" strokeWidth={2.2} /></div>
            ...
            <div className="container-wide grid lg:grid-cols-3 gap-8 lg:gap-10">
              <FadeIn>
                <div className="rounded-3xl bg-white border border-adobe shadow-card p-7 h-full">
                  ...
                  <ul className="space-y-3">{detail.whatsCovered.map((c) => (<li key={c} className="flex items-start gap-2.5 text-cocoa text-[0.95rem] leading-relaxed"><CheckCircle2 className="h-5 w-5 text-clay flex-shrink-0 mt-0.5" /><span>{c}</span></li>))}</ul>
                </div>
              </FadeIn>
              <FadeIn delay={0.08}>
              ...
              {related.map((r) => {
                ...
                return (
                  <Link key={r.slug} href={`/services/${r.slug}`} className="group p-6 rounded-3xl bg-white border border-adobe shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300">
                    <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-clay/10 text-clay group-hover:bg-clay group-hover:text-espresso transition-colors mb-3"><RIcon className="h-6 w-6" strokeWidth={2.2} /></div>
